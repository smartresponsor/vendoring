...
use App\ServiceInterface\Vendor\Statement\StatementExporterPDFInterface;
...
final class StatementExporterPDF implements StatementExporterPDFInterface
{
    ...
    {
        $dir = sprintf('var/statements/%s/%s/%s', date('Y', strtotime($dto->from)), date('m', strtotime($dto->from)), $dto->vendorId);
        if (!is_dir($dir)) { mkdir($dir, 0777, true); }
        $file = $dir.'/statement_'.date('Ymd', strtotime($dto->from)).'_'.date('Ymd', strtotime($dto->to)).'.pdf';
        $text = sprintf("Vendor Statement\nTenant: %s\nVendor: %s\nPeriod: %s .. %s\nCurrency: %s\n---\nOpening: %.2f\nEarnings: %.2f\nRefunds: %.2f\nFees: %.2f\nClosing: %.2f\n",
            $dto->tenantId, $dto->vendorId, $dto->from, $dto->to, $dto->currency,
            $data['opening'] ?? 0, $data['earnings'] ?? 0, $data['refunds'] ?? 0, $data['fees'] ?? 0, $data['closing'] ?? 0
        );
    ...
    private function writeMinimalPdf(string $path, string $text): void
    {
        ...
        $content .= "xref\n0 6\n0000000000 65535 f \n";
        $offsets = [0]
            + [0]; # placeholder; we'll recompute with simple fixed offsets (for brevity).

        // Minimal cross-ref table (approx for single-page). Not robust but enough for preview.
