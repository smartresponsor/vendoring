...
use App\Vendoring\ServiceInterface\Ops\VendorTransactionOperatorPageBuilderServiceInterface;
...
final class VendorTransactionOperatorController extends AbstractController
{
    public function __construct(
        ...
        private readonly VendorTransactionOperatorPageBuilderServiceInterface $pageBuilder,
    ...
    public function create(string $vendorId, Request $request): RedirectResponse
    {
        if ($this->isTwigFormsRuntimeAvailable()) {
            ...
            $form = $this->createForm(VendorTransactionCreateForm::class, $input, [
    ...
    public function updateStatus(string $vendorId, int $id, Request $request): RedirectResponse
    {
        ...
        if ($this->isTwigFormsRuntimeAvailable()) {
            ...
            $form = $this->createForm(VendorTransactionStatusUpdateForm::class, $input, [
        ...
        try {
            $this->transactionLifecycle->updateStatus($transaction, trim((string) $request->request->get('status', '')));
        } catch (InvalidArgumentException $exception) {
            return $this->redirectWithQuery($vendorId, 'error', $exception->getMessage());
        }
    ...
    {
        $createForm = $this->createForm(VendorTransactionCreateForm::class, new VendorTransactionCreateInputDTO(vendorId: $vendorId), [
            'action' => sprintf('/ops/vendor-transactions/%s/create', rawurlencode($vendorId)),
            'method' => 'POST',
            'csrf_protection' => false,
        ]);
