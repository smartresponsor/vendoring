...
final readonly class PayoutAccountRepository implements PayoutAccountRepositoryInterface
{
    ...
    public function get(string $tenantId, string $vendorId): ?PayoutAccount
    {
        ...
        return new PayoutAccount(
            $this->stringCell($r, 'id'),
            $this->stringCell($r, 'tenant_id'),
            $this->stringCell($r, 'vendor_id'),
            $this->stringCell($r, 'provider'),
            $this->stringCell($r, 'account_ref'),
            $this->stringCell($r, 'currency'),
            $this->boolCell($r, 'active'),
            $this->stringCell($r, 'created_at'),
        );
    }
    ...
    public function upsert(PayoutAccount $a): void
    {
        ...
        if ($this->intValue($exists) > 0) {
            $this->db->update(
                ...
                ['tenant_id' => $a->tenantId, 'vendor_id' => $a->vendorId],
            );
        } else {
            $this->db->insert(
                'payout_accounts',
