    ...
    public function loadFromExtension(string $extension, ?array $values = null): static
{
    if ($extension === 'framework') {
        @mkdir(dirname(__DIR__, 3).'/var', 0777, true);
        file_put_contents(
            dirname(__DIR__, 3).'/var/framework-loadfromextension-debug.log',
            "\n==== LOAD FROM EXTENSION framework ====\n"
    ...
    public function prependExtensionConfig(string $name, array $config): void
{
    if ($name === 'framework') {
        @mkdir(dirname(__DIR__, 3).'/var', 0777, true);
        file_put_contents(
            dirname(__DIR__, 3).'/var/framework-prepend-debug.log',
            "\n==== PREPEND framework ====\n"
