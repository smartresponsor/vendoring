...
foreach ($expectedScripts as $nameEntity => $commands) {
    if (vendoring_script_commands($composer, $nameEntity) !== $commands) {
        fwrite(STDERR, '[FAIL] composer script mismatch: '.$nameEntity.PHP_EOL);
        exit(1);
    }

    fwrite(STDOUT, '[OK] '.$nameEntity.' uses canonical smoke + unit/filter pattern'.PHP_EOL);
}
