...
final class VendorMonitoringSnapshotBuilderServiceTest extends TestCase
{
    public function testBuildAggregatesLogsMetricsBreakersAndProbeArtifacts(): void
    {
        ...
        file_put_contents($observabilityDir.'/runtime_metrics.ndjson', json_encode([
            'timestamp' => (new \DateTimeImmutable())->format(DATE_ATOM),
            'nameEntity' => 'statement_mail_failed_total',
        ]).PHP_EOL);
        ...
        $builder = new VendorMonitoringSnapshotBuilderService($observabilityDir, $root.'/fault-tolerance', $projectDir);
