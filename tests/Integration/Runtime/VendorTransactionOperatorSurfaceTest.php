...
final class VendorTransactionOperatorSurfaceTest extends TestCase
{
    public function testOperatorSurfaceRendersCreateAndStatusFlows(): void
    {
        ...
        try {
            $indexResponse = KernelRuntimeHarness::requestForm($kernel, 'GET', '/ops/vendor/transaction/vendor-ops');
            ...
            $createResponse = KernelRuntimeHarness::requestForm($kernel, 'POST', '/ops/vendor/transaction/create/vendor-ops', [
                'orderId' => 'order-ops-1',
            ...
            self::assertSame('/ops/vendor/transaction/vendor-ops?message=Transaction%20created.', $createResponse->headers->get('Location'));

            $reloadedResponse = KernelRuntimeHarness::requestForm($kernel, 'GET', '/ops/vendor/transaction/vendor-ops?message=Transaction%20created.');
            ...
            $statusResponse = KernelRuntimeHarness::requestForm($kernel, 'POST', '/ops/vendor/transaction/status/1', [
                'status' => 'authorized',
            ]);

            self::assertSame(302, $statusResponse->getStatusCode());
            self::assertSame('/ops/vendor/transaction/vendor-ops?message=Transaction%20status%20updated.', $statusResponse->headers->get('Location'));

            $authorizedResponse = KernelRuntimeHarness::requestForm($kernel, 'GET', '/ops/vendor/transaction/vendor-ops?message=Transaction%20status%20updated.');
    ...
    public function testOperatorSurfaceRendersValidationErrorFeedback(): void
    {
        ...
        try {
            ...
            self::assertSame(302, $createResponse->getStatusCode());
            self::assertSame('/ops/vendor/transaction/vendor-ops?error=order_id_required', $createResponse->headers->get('Location'));

            $errorResponse = KernelRuntimeHarness::requestForm($kernel, 'GET', '/ops/vendor/transaction/vendor-ops?error=order_id_required');
